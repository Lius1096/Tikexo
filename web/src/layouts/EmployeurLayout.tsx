...
import {
  ...
  BarChart2, FileSpreadsheet, Settings, LogOut, ShieldCheck, Lock,
} from 'lucide-react';
...
export function EmployeurLayout() {
  ...
  // Bloque l'accès uniquement une fois la donnée chargée ET non validée
  const kybBloque  = !kybLoading && kybData != null && !kybValide;
  const isOnKybPage = location.pathname === '/employeur/kyb';

  // Gate KYB : redirige vers /employeur/kyb si dossier pas encore validé
  useEffect(() => {
    if (kybBloque && !isOnKybPage) {
      navigate('/employeur/kyb', { replace: true });
    }
  ...
  return (
    <div className="flex h-screen font-sans">
      ...
      <nav className="w-[200px] min-w-[200px] bg-tikexo-primary flex flex-col flex-shrink-0" aria-label="Navigation TIKEXO">
        ...
        {/* Bandeau KYB dans la sidebar quand bloqué */}
        {kybBloque && (
          ...
          {NAV.map(({ section, items }) => (
            <div key={section}>
              ...
              {items.map(({ path, label, icon: Icon, exact }) => {
                ...
                if (locked) {
                  // Liens verrouillés : non cliquables, grisés avec cadenas
                  return (
                    <div
                      key={path}
      ...
      <main className="flex-1 overflow-auto bg-slate-100 flex flex-col">
        {/* Bandeau urgence deadline (affiché seulement sur les pages non-KYB) */}
