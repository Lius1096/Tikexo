...
export default function LandingCTA() {
  ...
  const title = cta?.title ?? "Votre première dotation\nen moins de 10 minutes.";
  const subtitle = cta?.subtitle ?? "Aucune intégration bancaire. Aucun matériel. Juste un compte, un virement Mobile Money, et vos salariés reçoivent leur wallet dès aujourd'hui.";
  const ctaPrimary = cta?.ctaPrimary ?? 'Créer mon compte entreprise';
  const ctaSecondary = cta?.ctaSecondary ?? "Parler à l'équipe";
  ...
  return (
    <section className="relative overflow-hidden bg-[#060E18] py-24 px-6 text-center">
      ...
      <div className="relative">
        ...
        <h2 className="text-5xl md:text-6xl font-black text-white mb-4 leading-[1.05] whitespace-pre-line">
          {title}
        </h2>
